...
// Allow other scripts to pause/resume the cloud shader
window.pauseClouds = function() { noLoop(); };
window.resumeClouds = function() { loop(); };
...
// Track whether a click hit the nav so p5's mousePressed can ignore it
let _navClicked = false;
document.addEventListener('mousedown', function(e) {
  const nav = document.getElementById('badge-nav');
  _navClicked = !!(nav && nav.contains(e.target));
}, true);
...
function mousePressed() {
  if (_navClicked) return;
  toggleNight();
}

function touchStarted() {
  if (_navClicked) return false;
  if (touches.length > 0) {
